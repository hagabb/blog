...
!
!  Content:
!     Implement image segmentation using Fortran DO CONCURRENT
!
! Compile for CPU:
!     ifx -free img_seg_do_concurrent.F90 -o img_seg_do_conc_cpu
!
! Compile for GPU using the OpenMP backend:
!     ifx -free img_seg_do_concurrent.F90 -o img_seg_do_conc_gpu \
...
program img_seg_do_conc_example
    ...
    do img_i = 1, images
        ...
        do concurrent (i = 1:n, j = 1:n, image(i,j) .ne. 0)
            if (i .eq. 1 .or. i .eq. n .or. &
                j .eq. 1 .or. j .eq. n) then
                edge_mask(i,j) = .true.
            else
                if (any(image(i-1:i+1, j-1:j+1) .eq. 0)) edge_mask(i,j) = .true. 
            endif
        enddo
        ...
        if (img_i .ne. 1) total_time = total_time + cycle_time
